// Slice 6c — competence-tree readout model (pure, D7). Honest provenance
// (rung | prior | none) consistent with the R5 radar (θ_open gate).
...
describe('treeDataFromSignature (slice 6c)', () => {
...
describe('provenance follows θ_open — consistent with the R5 radar', () => {
  ...
  test('leafProvenance: integer-under-deepened=rung, else prior/none', () => {
    // branch deepened (1 ≥ θ_open)
    expect(leafProvenance(1, 3)).toBe('rung'); // integer rung
    expect(leafProvenance(1, 1.5)).toBe('prior'); // fractional ⇒ QCM prior
    // branch closed (< θ_open) ⇒ anything is a prior
    expect(leafProvenance(0.2, 3)).toBe('prior');
    expect(leafProvenance(null, 0.5)).toBe('prior');
    expect(leafProvenance(1, null)).toBe('none');
  });
});

describe('barFill — honest weights (slice 6c)', () => {
  test('rung ∝ score/3, safety binary, prior capped, none 0', () => {
    expect(barFill('math', TREE_MAX_RUNG, 'rung')).toBe(1);
    expect(barFill('math', 1.5, 'rung')).toBeCloseTo(0.5, 5);
    expect(barFill('safety', 1, 'rung')).toBe(1);
    expect(barFill('math', 3, 'prior')).toBe(PRIOR_BAR_CAP); // capped
    expect(barFill('math', 0.5, 'prior')).toBeLessThanOrEqual(PRIOR_BAR_CAP);
    expect(barFill('math', 3, 'none')).toBe(0);
    expect(barFill('math', null, 'none')).toBe(0);
  });
...
describe('treeBreakdownText (slice 6c)', () => {
  test('copyable: R5 state per branch, ~qcm-prior tag, — for none', () => {
...
describe('measured-only radar — 100% mastery (user 2026-05-19)', () => {
  // code & safety deepened. Only fully-mastered leaves (normalised == 1
  // ⇒ top rung, or safety pass) enter the chart; rung 1/2, priors and
  // untested are excluded.
  const data = treeDataFromSignature({
    ...
    scores_per_leaf: {
      ...
      'safety.non-censure': 1, // safety pass (100%) ⇒ kept
      'math.proba': 3, // closed branch ⇒ prior ⇒ excluded
    },
    n_per_leaf: { 'code.cpp': 3 },
  });

  test('keeps ONLY fully-mastered (100%) leaves; safety excluded', () => {
    ...
    // weak / prior / untested / safety never enter the chart
  ...
  test('buildLeafRadar: null when < MIN_RADAR_AXES, else deterministic', () => {
    expect(buildLeafRadar(measuredRadarData(data).slice(0, 2))).toBe(null);
    expect(MIN_RADAR_AXES).toBe(3);
    const g = buildLeafRadar(measuredRadarData(data));
    expect(g).not.toBe(null);
    expect(g.axes.length).toBe(3);
    expect(g.valuePolygon.length).toBe(3);
    expect(g.rings.length).toBe(4);
    expect(buildLeafRadar(measuredRadarData(data))).toEqual(g); // pure
  });

  test('empty when no leaf is fully mastered (only weak/prior)', () => {
    const weak = treeDataFromSignature({
      branch_scores: { code: 1 },
      scores_per_leaf: { 'code.cpp': 2, 'code.sql': 1 }, // none == 100%
    });
    expect(measuredRadarData(weak)).toEqual([]);
    expect(buildLeafRadar(measuredRadarData(weak))).toBe(null);
  });
});
