...
**No in-app chat surface** — once a model is launched, TagSpaces opens `http://127.0.0.1:<port>/` in the user's default browser (the native llama-server UI). The conversation surface belongs to external clients (deer-flow, aider, Claude Desktop). Don't reintroduce a ChatDialog or an embedded terminal — that path has been deliberately removed.
